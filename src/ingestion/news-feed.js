...
class NewsFeed {
    ...
    async fetchMarketNews(asset) {
        ...
        const url = `https://newsapi.org/v2/everything?q=${encodeURIComponent(query)}&language=en&sortBy=publishedAt&pageSize=20&apiKey=${this.apiKey}`;
        const res = await axios_1.default.get(url);
        return res.data.articles.map((a) => {
            var _a;
            return ({
                title: a.title,
                source: ((_a = a.source) === null || _a === void 0 ? void 0 : _a.name) || "Unknown",
                url: a.url,
