...
exports.candlesRouter = exports.CandlesRouter = exports.CandleRouterError = void 0;
const axios_1 = __importDefault(require("axios"));
class CandleRouterError extends Error {
...
exports.CandleRouterError = CandleRouterError;
...
class CandlesRouter {
    ...
    async getCandles(req) {
        var _a;
        const normalized = Object.assign(Object.assign({}, req), { symbol: req.symbol.trim().toUpperCase(), limit: (_a = req.limit) !== null && _a !== void 0 ? _a : DEFAULT_LIMIT });
    ...
    async tryProviders(providers, req) {
        const errors = [];
    ...
    async fetchBinanceCandles(req) {
        var _a;
        const interval = this.mapIntervalToBinance(req.interval);
        const symbol = this.normalizeCryptoSymbolForBinance(req.symbol);
        const response = await axios_1.default.get(`${BINANCE_MARKET_DATA_BASE}/api/v3/klines`, {
            params: {
                symbol,
                interval,
                limit: Math.min((_a = req.limit) !== null && _a !== void 0 ? _a : DEFAULT_LIMIT, 1000),
            },
            timeout: 12000,
        });
        const candles = response.data.map((row) => ({
    ...
    async fetchOkxCandles(req) {
        var _a, _b;
        const instId = this.normalizeCryptoSymbolForOkx(req.symbol);
        const bar = this.mapIntervalToOkx(req.interval);
        const response = await axios_1.default.get(`${OKX_BASE}/api/v5/market/candles`, {
            params: {
                instId,
                bar,
                limit: Math.min((_a = req.limit) !== null && _a !== void 0 ? _a : DEFAULT_LIMIT, 300),
            },
            timeout: 12000,
        });
        const rows = (_b = response.data) === null || _b === void 0 ? void 0 : _b.data;
        ...
        const candles = rows.map((row) => {
            ...
            return ({
                ...
                low: Number(row[3]),
                close: Number(row[4]),
                volume: Number((_a = row[5]) !== null && _a !== void 0 ? _a : 0),
            });
        });
        return {
            provider: "okx",
    ...
    async fetchAlphaVantageCandles(req) {
    ...
    async fetchAlphaVantageForex(req, apiKey) {
        ...
        const timeSeriesKey = Object.keys(response.data).find((k) => k.toLowerCase().includes("time series"));
    ...
    async fetchAlphaVantageAsset(req, apiKey) {
    ...
    async fetchEiaEnergySeries(req, apiKey) {
        var _a, _b;
        ...
        const response = await axios_1.default.get(`${EIA_BASE}/${seriesPath}`, {
            params: {
                api_key: apiKey,
                length: req.limit,
        ...
        const data = (_b = (_a = response.data) === null || _a === void 0 ? void 0 : _a.response) === null || _b === void 0 ? void 0 : _b.data;
    ...
    normalizeCryptoSymbolForBinance(symbol) {
        const upper = symbol.toUpperCase();
        return upper.endsWith("USDT") ? upper : `${upper}USDT`;
    }
    normalizeCryptoSymbolForOkx(symbol) {
    ...
    mapSymbolForAlphaVantage(symbol, assetClass) {
        var _a;
        const upper = symbol.toUpperCase();
        const customMap = {
            SPX: "SPY",
        ...
        if (assetClass === "metal" && upper === "XAUUSD")
            return "GLD";
        if (assetClass === "metal" && upper === "XAGUSD")
            return "SLV";
        return (_a = customMap[upper]) !== null && _a !== void 0 ? _a : upper;
    }
    mapSymbolForStooq(symbol, assetClass) {
        var _a;
        const upper = symbol.toUpperCase();
        if (assetClass === "stock") {
        ...
        return (_a = map[upper]) !== null && _a !== void 0 ? _a : upper.toLowerCase();
    }
    mapIntervalToBinance(interval) {
        const map = {
            "1min": "1m",
            "5min": "5m",
            "15min": "15m",
    ...
    mapIntervalToAlphaVantage(interval) {
        const map = {
            "1min": "1min",
            "5min": "5min",
            "15min": "15min",
    ...
    mapEiaSeries(symbol) {
        ...
        return (_a = map[upper]) !== null && _a !== void 0 ? _a : null;
    }
    parseEiaPeriod(period) {
...
exports.CandlesRouter = CandlesRouter;
exports.candlesRouter = new CandlesRouter();
