export interface WeatherShockSignal {
  risk: "LOW" | "MEDIUM" | "HIGH"
  impact: "NEUTRAL" | "RISK_ON" | "RISK_OFF"
  assets: string[]
  regions: string[]
  severity: number
...
export function analyzeWeatherShock(events: any[]): WeatherShockSignal {
  ...
  for (const e of events) {
    ...
    // REGION
    if (text.includes("us") || text.includes("america")) regions.push("US")
    if (text.includes("china")) regions.push("CHINA")
    if (text.includes("europe") || text.includes("eu")) regions.push("EUROPE")
    if (text.includes("gulf")) regions.push("US_GULF")
  ...
  let impact: "NEUTRAL" | "RISK_ON" | "RISK_OFF" = "NEUTRAL"

  if (severity >= 60) impact = "RISK_OFF"
  else if (severity >= 20) impact = "NEUTRAL"
